...
def _find_graph(parent_widget):
    """Walk up the widget hierarchy to find the nearest object that owns a graph.

    Falls back to scanning top-level windows via duck typing so that no import
    of MainWindow is required (avoids a circular dependency with gui.main_window).
    """
    p = parent_widget
    while p:
        if hasattr(p, 'graph'):
    ...
    try:
        mw = next(
            (w for w in QApplication.topLevelWidgets()
             if hasattr(w, 'graph') and hasattr(w, 'scene')),
            None,
        )
        if mw:
            return mw.graph
