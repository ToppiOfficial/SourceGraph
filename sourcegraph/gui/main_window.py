...
from sourcegraph.sys.graph    import Graph
from sourcegraph.gui.node_editor import NodeEditorScene, NodeEditorView, MinimapWidget
from sourcegraph.gui.panels.manager import PanelManager
from sourcegraph.sys.registry import NODE_CLASS_MAPPINGS
from sourcegraph.gui.theme import *
from sourcegraph.gui.logger import log, Level
from sourcegraph.gui.widgets.basic_shapes import ShapeDrawer, IconColors
...
from sourcegraph.gui.menu.plugin_manager_dialog import PluginManagerDialog
from sourcegraph.sys.paths import app_root
from sourcegraph.sys.plugins import PluginLoader
from sourcegraph.sys.registry import get_default_registry
...
class MainWindow(QMainWindow):
    def __init__(self, on_progress: callable | None = None) -> None:
        super().__init__()
        self.setWindowTitle("SourceGraph")
    ...
    def _update_workspace_list(self, menu=None) -> None:
        ...
        workspace_dir = os.path.join(str(app_root()), "workspace")
    ...
    def _load_workspace(self, filename: str) -> None:
        ...
        workspace_path = os.path.join(str(app_root()), "workspace", filename)
        self._load_layout(workspace_path)

    def _reset_layout(self) -> None:
        # Load default.json layout
        default_path = os.path.join(str(app_root()), "workspace", "default.json")
    ...
    def closeEvent(self, event) -> None:
        ...
        if self._maybe_save():
            ...
            autosave_path = os.path.join(str(app_root()), "workspace", "autosave.json")
    ...
    def _load_builtin_types(self) -> None:
        """Load built-in port type registrations from the sourcegraph.types package."""
        import pkgutil
        import importlib
        import sourcegraph.types as _types_pkg
        for _finder, modname, _ispkg in pkgutil.iter_modules(
                _types_pkg.__path__, _types_pkg.__name__ + '.'):
            if modname.split('.')[-1].startswith('_'):
                continue
            try:
                importlib.import_module(modname)
            except Exception as exc:
                print(f"[BuiltinTypes] Failed to load '{modname}': {exc}")
    ...
    def _update_window_title(self) -> None:
        ...
        if self._project_path:
            if self._show_full_path_in_title:
                # Show full path
                self.setWindowTitle(f"SourceGraph - {self._project_path}")
            else:
                # Show just filename
                project_name = os.path.basename(self._project_path)
                self.setWindowTitle(f"SourceGraph - {project_name}")
        else:
    ...
    def _save_as(self) -> bool:
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Graph As", self._project_path or "", "SourceGraph (*.srcgraph);;SourceSubgraph (*.srcsubgraph);;All (*)")
        if path:
            self._project_path = path
    ...
    def _open(self) -> None:
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, "Open Graph", "", "SourceGraph Files (*.srcgraph *.srcsubgraph);;All (*)")
        if path:
            self._nav_root = None
            self._current_nav = None
    ...
    def _setup_crash_reporting(self):
        ...
        try:
            for f in os.listdir(temp_dir):
                if f.startswith("sourcegraph_crash_") and f.endswith(".log"):
        ...
        # Cleanup empty logs in project crash dir (only if dir exists from a previous crash)
        ...
        try:
            self._fatal_log_temp = tempfile.NamedTemporaryFile(mode="w", prefix="sourcegraph_crash_", suffix=".log", delete=False)
