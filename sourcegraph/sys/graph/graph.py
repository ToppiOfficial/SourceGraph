...
from sourcegraph.sys.graph.stores import get_all_store_specs
...
class Graph:
    ...
    def execute_with_context(self, context: ExecutionContext) -> dict[str, ExecutionResult]:
        from sourcegraph.sys.execution.engine import StandardExecutionEngine
        engine = StandardExecutionEngine()
        return engine.execute(self, context)

    def to_dict(self) -> dict:
        from sourcegraph.sys.registry.nodes import get_default_registry
    ...
    def load_dict(self, data: dict, registry: dict | None = None) -> None:
        from sourcegraph.sys.registry.nodes import get_default_registry, NODE_CLASS_MAPPINGS
