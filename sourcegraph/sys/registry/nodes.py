        ...
        if entry.is_dir() and not entry.name.startswith("__"):
            for file in os.scandir(entry.path):
                if file.name.endswith(".py") and not file.name.startswith("__"):
                    module_name = f"sourcegraph.nodes.{entry.name}.{file.name[:-3]}"
                    try:
...
def discover_builtin_nodes() -> None:
    """Discover and register all nodes in the bundled sourcegraph.nodes package.

    Uses pkgutil so this works from source AND when frozen by PyInstaller.
    """
    import pkgutil
    import sourcegraph.nodes as _nodes_pkg

    NODE_CATEGORIES.clear()

    for _finder, subpkg_name, _ispkg in pkgutil.iter_modules(
            _nodes_pkg.__path__, _nodes_pkg.__name__ + '.'):
        try:
            subpkg = importlib.import_module(subpkg_name)
        except Exception as e:
            print(f"[Node Registration] Failed to import {subpkg_name}: {e}")
            continue
        for _finder2, modname, _ispkg2 in pkgutil.iter_modules(
                subpkg.__path__, subpkg_name + '.'):
            try:
                ...
                for _name, obj in inspect.getmembers(module):
                    if (inspect.isclass(obj) and
                            ...
                            obj.__name__ not in NODE_CLASS_MAPPINGS):
                        if not hasattr(obj, 'CATEGORY'):
                            obj.CATEGORY = subpkg_name.split('.')[-1].title()
                        register_node(obj)
            except Exception as e:
                print(f"[Node Registration] Failed to load module {modname}: {e}")
